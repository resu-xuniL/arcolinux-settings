...
manage_one() {
    ...
    if [[ ${action_type} == "steps" ]]; then
        ...
        if [[ ${action_type} == "install" ]]; then
            ...
            exec_log "${sudo_str}cp '${INSTALL_DIRECTORY}'/${target} ${destination}" "${GREEN}[+]${RESET} Copying [${YELLOW}${file_name[1]}${RESET}] ${file_or_folder} to [${YELLOW}${destination/${HOME}/'~'}${RESET}] folder"
        fi
    fi
}