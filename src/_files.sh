...
set_config_files() {
    ...
    if [[ ${packages} =~ "conky/conky-sessionfile" ]]; then
        ...
        exec_log "chmod +x ${INSTALL_DIRECTORY}/conky/wam_fetch_icon.sh" "${GREEN}[+]${RESET} Setting [${YELLOW}execution permission${RESET}] on [${YELLOW}wam_fetch_icon.sh${RESET}] file"
        ...
        exec_log "chmod +x ${INSTALL_DIRECTORY}/conky/wam_color_switch.sh" "${GREEN}[+]${RESET} Setting [${YELLOW}execution permission${RESET}] on [${YELLOW}wam_color_switch.sh${RESET}] file"
